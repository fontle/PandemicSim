...
class _RadioFrame(ttk.LabelFrame):
    ...
    def __init__(self, root, title: str, options: dict) -> None:
        ...
        for col, val in enumerate(self.options.keys()):
            button = ttk.Radiobutton(self,text=val,value=self.options[val],variable=self.var)
...
class _Slider(ttk.LabelFrame):
    ...
    def __init__(self, root, title: str, from_: int, to: int, default: int) -> None:
        ...
        self.slider = ttk.Scale(self, from_ = from_, to = to, orient = 'horizontal', command=lambda _:self.set_label())
...
class _Menu(tk.Tk):
    ...
    def __init__(self, config_addr = 'config.json'):
        ...
        self.theme_chooser = _RadioFrame(self, 'Simulation Theme', {'Light':'light', 'Dark':'dark'})
        ...
        self.lethality = _RadioFrame(self.pathogen_frame, 'Lethality', {'Low': '0.001', 'Medium': '0.002', 'High': '0.005'})
        self.lethality.grid(row=0, column=0, pady=5)

        self.curability = _RadioFrame(self.pathogen_frame, 'Curability', {'Low': '0.001', 'Medium': '0.0025', 'High': '0.005'})
        self.curability.grid(row=1, column=0, pady=5)

        self.catchment = _RadioFrame(self.pathogen_frame, 'Catchment', {'Low': '1', 'Medium': '5', 'High': '10'})
        self.catchment.grid(row=2, column=0, pady=5)

        self.infectiousness = _RadioFrame(self.pathogen_frame, 'Infectiousness', {'Low': '0.03', 'Medium': '0.1', 'High': '0.25'})
        ...
        self.migrations = _RadioFrame(self.mitigation_frame, 'Migrations', {'Low': '0.01', 'Medium': '0.1', 'High': '0.25'})
        self.migrations.grid(row=0, column=0, padx=5, pady=5)

        self.movements = _RadioFrame(self.mitigation_frame, 'Movements', {'Low': '0.01', 'Medium': '0.05', 'High': '0.15'})
